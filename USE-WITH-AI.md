...
그다음 `hooks install --global --agents codex,claude,gemini --provider vercel --security-only`와 `hooks status --agents codex,claude,gemini --json`을 실행해. 기본 경로는 Codex `~/.codex/config.toml`, Claude `~/.claude/settings.json`, Gemini `~/.gemini/settings.json`이지만 `CODEX_HOME`, `CLAUDE_CONFIG_DIR`, `GEMINI_CLI_HOME`이 있으면 그 공식 사용자 지정 루트를 따라야 해. 기존 설정을 보존하고 status의 에이전트별 `configRoot`, `configRootSource`와 공통 `nodeExecutableVerified`, `hostPlatform`, `home`, `isWSL`, `installationScope`, `timeoutSeconds`, `hostTimeoutMayFailOpen`을 확인해. native Windows라면 `windowsPowerShellSupported:true`, `windowsVercelExecutableVerified:true`, `windowsVercelVersion:"59.10.0"`, `windowsVercelExecutable`, 두 executable SHA-256까지 확인해. 설치기가 PowerShell `Get-Command vercel -All -CommandType Application`의 exact `vercel.cmd`를 찾아 고정해야 하며 PATH 이름이나 `vercel.ps1`를 신뢰하면 안 돼. install/status 종료 코드 3과 `configured:true, activation:unknown, ready:false`는 설정을 기록했지만 호스트 활성화를 아직 모른다는 정상적인 보수 결과이지 PASS가 아니야. Windows와 WSL은 각각 설치해. Codex/Claude/Gemini 실행 플래그와 프로젝트·시스템 설정이 훅을 끌 수 있으므로 재시작·신뢰 뒤 실제 에이전트 셸에서 안전한 invalid target promote를 요청해. Bash는 `vercel promote https://dcheck-hook-challenge.invalid`, PowerShell은 status에 보인 실제 경로를 넣은 `& '<exact windowsVercelExecutable>' promote https://dcheck-hook-challenge.invalid`야. Vercel CLI 실행 전에 dorms-check가 exit 2로 차단하는 것을 실제로 보지 못하면 배포를 진행하지 마.

code strict가 PASS이고 영수증이 유효할 때 현재 HEAD 40자리를 다시 읽어. macOS/Linux/WSL은 그 SHA를 직접 넣은 `vercel --prod --skip-domain --meta githubDeployment=1 --meta githubCommitSha=<실제 HEAD 40자리> --yes`를 실행해. native Windows PowerShell은 변수를 쓰지 말고 status의 실제 경로와 SHA를 문자열에 직접 넣은 `& '<exact windowsVercelExecutable>' --prod --skip-domain --meta githubDeployment=1 --meta githubCommitSha=<실제 HEAD 40자리> --yes`를 실행해. `npx`·`pnpm`·`bunx`, package manager·script/workspace, task runner, 셸·Node·Python runtime 스크립트, 다른 경로 실행 파일, command substitution, `eval`, caret/backtick, variable, splatting, 동적 cmdlet, 복합 명령, pipe, redirect, `--cwd`, `--local-config`, `--project`, `--scope`, `--env`, `--build-env`, `--prebuilt`, `--archive`를 쓰지 마. stdout의 정확한 Deployment URL을 캡처해 그 URL 하나를 `--url`과 `--vercel-deployment`에 똑같이 넣은 live strict와 `gate verify`를 실행해. 모두 PASS일 때 Bash는 `vercel promote <정확한 literal URL>`, PowerShell은 `& '<exact windowsVercelExecutable>' promote <정확한 literal URL>` 단일 명령만 실행해.
