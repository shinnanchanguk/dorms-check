---
name: dorms-check
description: 교사가 바이브코딩으로 만든 웹앱(주로 Next.js·Supabase·Vercel 또는 정적 사이트)을 스스로 점검하고, 완전히 안전해질 때까지 자기 AI와 함께 고치는 보안·개인정보 점검 코치. 코드는 고치지 않고 평가·안내·증빙만 한다. 결정적 스캐너(헤더·SSL·노출·CORS·RLS 실측)로 판정해 약한 모델이 "문제 없다"고 착각하는 걸 구조적으로 막는다. 통과하면 도름스 서버가 스스로 재검증해 인증마크("도름스 보안 체크리스트 충족"·"학운위 심사 준비 완료")를 발급한다. 트리거 — "내 앱 안전한지 검사", "보안 검토", "개인정보 점검", "학운위 심사 준비", "에듀집 체크리스트", "도름스 인증마크", "dorms-check".
---
