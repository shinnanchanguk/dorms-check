---
name: dorms-check
description: 교사가 바이브코딩으로 만든 웹앱(주로 Next.js·Supabase·Vercel 또는 정적 사이트)을 세 축으로 점검하고 고치도록 돕는 코치. ① 보안 점검(security) ② 학운위·개인정보 준비(edzip) ③ 내 앱 비법·저작권 보호(protection). 보안·학운위는 코드를 고치지 않고 평가·안내·증빙만 하고, 보호 축의 적용 단계만 사용자가 승인한 계획 해시와 --confirm-apply 가 있을 때 파일을 바꾼다(자동 배포 없음). 결정적 스캐너(헤더·SSL·노출·CORS·RLS 실측·산출물 검사)로 판정해 약한 모델이 "문제 없다"고 착각하는 걸 구조적으로 막는다. 통과하면 도름스 서버가 스스로 재검증해 인증마크("도름스 보안 체크리스트 충족"·"학운위 심사 준비 완료")를 발급한다. 트리거 — "내 앱 안전한지 검사", "보안 검토", "개인정보 점검", "학운위 심사 준비", "에듀집 체크리스트", "도름스 인증마크", "내 앱 비법 보호", "복제 방지", "저작권 보호", "dorms-check".
---
...
- **세 축의 역할 (철학 계약)**: security(보안 점검)·edzip(학운위·개인정보 준비)은 **검사만 한다**. protection(내 앱 비법·저작권 보호)의 **적용 단계만** 파일을 바꾸며, 그것도 사용자가 승인한 **계획 해시와 `--confirm-apply` 플래그가 있을 때만**이다. **자동 배포는 하지 않는다.** 코드 수정(보안 항목 고치기, 비법을 서버로 옮기기)은 교사의 AI(당신)가 교사 동의 하에 한다.
