...
6. `manifest.json`의 파일 해시, 출처 접속 상태, `piiStored:false`, `automaticSubmission:false`를 확인한다. HWPX는 `skills/hwpx-edit/SKILL.md`의 검증을 따른다. 한글에서 표 칸의 글자가 겹쳐 보이면 문서 전체를 드래그해 복사한 다음, 전체가 선택된 그 상태에서 바로 붙여넣으면 해결된다고 반드시 안내한다.
