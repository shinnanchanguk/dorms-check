...
- 구조 검사를 통과했는데도 한글에서 표 칸의 글자가 겹쳐 보이면, 사용자에게 원본 복사본을 먼저 남기게 한다. 문서 전체를 드래그하거나 Ctrl+A로 선택해 복사한 뒤 새 빈 한글 문서에 다시 붙여넣으면 한글이 줄 배치를 다시 계산해 겹침이 풀린다고 안내한다. 붙여넣은 문서는 표와 쪽 나눔을 확인한 뒤 새 이름으로 저장하게 한다.
