...
`dcheck`는 code 영수증, 현재 `.vercel/project.json`, `vercel inspect <URL|ID> --format=json`, 내부 read-only `GET /v13/deployments/<ID>` metadata를 함께 확인합니다. 사용자에게 임의 `vercel api` 실행을 허용하는 것이 아니라 고정 dcheck가 exact deployment ID와 linked org로 GET 한 번만 수행합니다. 두 응답의 URL/ID, `READY`, production target뿐 아니라 `githubDeployment=1`, exact `githubCommitSha`, 배포 source Git SHA가 현재 HEAD인지, Vercel projectId와 ownerId/orgId가 서명된 링크와 같은지 확인해 영수증에 묶습니다. `githubDeployment=1`과 exact `githubCommitSha`가 없거나 프로젝트 정보가 없으면 `INCOMPLETE`, 값이 다르면 `BINDING_MISMATCH`입니다. 배포에 Git 연동이 직접 붙인 `gitSource.sha`는 있으면 현재 HEAD와 대조하고, 없으면 검사를 세우지 않고 결속 근거를 `declared`로 기록합니다. 이 필드는 Git 연동이 만든 배포에만 붙기 때문에, `vercel --prod --skip-domain`으로 본인이 직접 올리는 경로에서는 비어 있는 것이 정상입니다. 화면의 `Source` 표시는 이 필드가 아니라 `meta.githubCommitSha`입니다. 결속 근거는 `--json` 출력의 `deployment.gitBinding`에 `canonical` 또는 `declared`로 남습니다. 배포자를 신뢰할 수 없는 자리에서는 `--require-canonical-git-source`로 `gitSource.sha`가 있는 배포만 통과시킬 수 있습니다. 최초 URL이 다른 origin으로 redirect되어 그쪽 내용을 검사하는 경우도 바인딩 불일치로 중단합니다. HTTP 강제 이동도 `https://evil.example` 같은 외부 origin이나 malformed Location은 실패하며, 검사 중인 정확한 배포 origin의 HTTPS로 갈 때만 통과합니다.
